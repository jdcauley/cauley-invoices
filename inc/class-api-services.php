...
if ( ! class_exists( 'Mediavine\API_Services' ) ) {
	...
	class API_Services {
		...
		function prepare_item_for_response( $item, $request, $relationships = array() ) {
			...
			if ( isset( $response_data['instructions'] ) ) {
				$response_data['instructions_rendered'] = do_shortcode( $response_data['instructions'] );
			}

			if ( isset( $response_data['notes'] ) ) {
				$response_data['notes_rendered'] = do_shortcode( $response_data['notes'] );
			}
